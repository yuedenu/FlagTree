        ...
        if g >= 1:
            ...
            if g1 < n_sub:
                ...
                s1r_bp = tl.make_block_ptr(stage1Res + cid * (PP * BLOCK_M * BLOCK_N) + pp1 * (BLOCK_M * BLOCK_N),
                                           (BLOCK_M, BLOCK_N), (BLOCK_N, 1), (0, 0), (BLOCK_M, BLOCK_N), (1, 0))
                tile_copy(tl.load(s1r_bp), p_l1, [CBM, CBN])
                # V[j1] -> v_l1 -> L0 slot 1
...
def dump_tileir(path=None, num_iters=32, is_causal=False):
    """Compile the kernel to TTIR (containing tile.* ops) and write it to `path`.

    Returns the MLIR string. Requires no NPU/GPU — pure front-end compilation.
    """

...
_optimized = _load_module(
    "matrix_multiplication_optimized",
    os.path.join(_script_dir, "13-matrix-multiplication-optimized.py"),
)
...
def main():
    ...
    if not args.no_check:
        ...
        # optimized: uses float16, compare in float32
        out_optimized = _optimized.triton_matmul(mat_a, mat_b)
        for label, out in [("pytorch", ref), ("flaggems", out_flaggems)]:
            try:
        ...
        try:
            torch.testing.assert_close(ref, out_optimized, rtol=1e-2, atol=1e-2)
            print("  [optimized] correctness check PASSED")
        except AssertionError as e:
            print(f"  [optimized] correctness check FAILED: {e}")
        print()
    ...
    # optimized kernel uses bfloat16 and a two-arg interface; wrap to match.
    def optimized_call(a, b, _num_cores):
        return _optimized.triton_matmul(a, b)

    # (label, call_fn, mat_a override, mat_b override)
    # None for mat overrides means use the default float16 tensors.
    _kernel_envs = [
        ("pytorch             ", torch_call, mat_a, mat_b, None),
        ("flaggems_matmul     ", _flaggems.call, mat_a, mat_b, None),
        ("optimized      ", optimized_call, mat_a, mat_b, None),
    ]

    results = {}
    for label, fn, a, b, skip_cse in _kernel_envs:
        #with _env("COMMONIR_SKIP_CSE", skip_cse):
        if args.mode == "kernel":
            median = bench_fn(fn, a, b, num_cores, warmup=args.warmup, rep=args.rep)
            tfl = _tflops(M, N, K, median)
            print(f"  {label}  median={median:7.3f} ms  |  {tfl:.3f} TFLOPS")
        else:
            median, mean, mn, mx = bench_fn(fn, a, b, num_cores, device, warmup=args.warmup, rep=args.rep)
            tfl = _tflops(M, N, K, median)
    ...
    opt_ms = results["optimized      "]
    #native_ms = results["native_matmul       "]
    #db_ms     = results["double_buffer_matmul"]

    print()
    for label, ms in [("flaggems_matmul", fg_ms), ("optimized", opt_ms)]:

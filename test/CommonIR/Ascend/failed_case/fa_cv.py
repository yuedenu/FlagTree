...
def _mm1_qkt(
    ...
    DMA + MMA sequence for each KV block.  Serial version: single workspace
    slot per core (no ring index).
    """
...
def _vec1_softmax(
    ...
    for cb_idx in range(CB):
        ...
        # neg_max_prv = previous block's running max (opposite slot)
        # neg_max_new = min of current block's -max*scale vs previous running max
        block_row_max = tl.max(attn_score_block, axis=-1, keep_dims=True)
        neg_max_prv = tl.where(cur_parity == 0, neg_max_odd, neg_max_even)
        neg_max_new = tl.minimum(-block_row_max * sm_scale, neg_max_prv)
...
def flash_attention_fwd_serial_kernel(
    ...
    for g in range(num_global_tasks):
        ...
        # -----------------------------------------------------------------
        #  Cube scope — MM2: P * V -> workspace_pv
        # -----------------------------------------------------------------
        with tle.scope(core_mode="cube"):
            _mm2_pv(
                V,
        ...
        # -----------------------------------------------------------------
        #  Vector scope — Vec2: accumulate pv into acc_o; write output on last block
        # -----------------------------------------------------------------
        with tle.scope(core_mode="vector"):

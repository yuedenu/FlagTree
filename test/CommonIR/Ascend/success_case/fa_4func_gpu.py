...
# =============================================================================
#  Serial single-slot FA kernel — no sync, no tle.scope (CommonIR / tle.dsa form)
#
...
def dump_commonir(path=None, ttir_path=None, num_kv_blocks=32, is_causal=False):
...
def dump_hivm(path=None, is_causal=False):
    ...
    commonir_mlir = dump_commonir(path=None, is_causal=is_causal)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dump-mlir", nargs="?", const="", default=None,
                        help="Dump intermediate CommonIR to PATH and exit; no device needed.")
    ...
    if args.dump_mlir is not None:
        dump_commonir(path=(args.dump_mlir or None), is_causal=args.causal)
        raise SystemExit(0)

...
#  Host-side launch
# =============================================================================
def call(mat_a, mat_b, num_cores=_DEFAULT_NUM_CORES, debug_compile=False):
    m = mat_a.shape[0]
    k = mat_a.shape[1]
    n = mat_b.shape[1]
    ...
    matmul_kernel[(num_cores, )](mat_a, mat_b, mat_c, m, n, k, num_cores, BLOCK_M=BLOCK_M, BLOCK_N=BLOCK_N,
                                 BLOCK_K=BLOCK_K,
                                 custom_pipeline="builtin.module(multi-buffer-pipeline,code-motion)", **compile_options)
    return mat_c
...
if __name__ == "__main__":
    ...
    mat_c = call(mat_a, mat_b, num_cores, debug_compile=args.debug_compile)
